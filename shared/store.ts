  ...
  persist(
    (set, get) => ({
      ...
      applyMergedSnapshot: (cloud, savedAt) =>
        set((local) => {
          ...
          try {
            if (readFlag("tokmato:v8-just-upgraded")) {
    ...
    },
    {
      ...
      migrate: (persistedState, version) => {
        if (version < 8) {
          try {
            clearFlag("tokmato:guide-seen");
            clearFlag("tokmato:welcome-granted");
            writeFlag("tokmato:v8-just-upgraded", "1");
          } catch {}
        ...
        if (version < 6) {
          ...
          if (isExistingUser) {
            try {
              writeFlag("tokmato:guide-seen", "1");
            } catch {}
          }
      ...
      skipHydration: true,
      storage: createJSONStorage(() => getSharedStorage()),
